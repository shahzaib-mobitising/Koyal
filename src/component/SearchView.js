...
function SearchView() {
    ...
    const trackView = <Grid container spacing={0}>{searchData.searchDataTrack.map(track =>
        <Grid item xs={3} key={track.TrackId}>
            <List className="trackListView">
                <ListItem alignItems="flex-start">
                    ...
                    <Link onClick={handleClose} component={Link} to={`/track/` + track.TrackId + `/` + track.Name}>  <ListItemText primary={track.Name} />
                    </Link>
                </ListItem>
    ...
    const albumView = <Grid container spacing={7}> {searchData.searchDataAlbum.map((i, index) => (
        <Grid key={i.Id} item xs={2} className="album_grid">
            <Card className="viewMoreBox">
                <Link onClick={handleClose} component={Link} to={`/album/` + i.Id + `/` + i.Name}>
                    <CardActionArea>
                        <CardMedia
                            component="img"
                            alt={i.Name}
    ...
    const artistView = <Grid container spacing={7}> {searchData.searchDataArtist.map((i, index) => (
        <Grid key={i.Id} item xs={2} className="album_grid">
            <Card className="viewMoreBox">
                <Link onClick={handleClose} component={Link} to={`/artist/` + i.Id + `/` + i.Name}>
                    <CardActionArea>
                        <CardMedia
                            component="img"
                            alt={i.Name}
    ...
    return (
        ...
        <div className="searchMainBox">
            ...
            <Dialog fullScreen open={open} onClose={handleClose} TransitionComponent={Transition} className="searchDialogueBody">
